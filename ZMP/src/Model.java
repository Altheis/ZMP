...
public class Model {
	public final char player1 = 'w';
	public final char player2 = 'b';
	...
	private ArrayList<Move> checkMove(char[][] board, int i, int j) {
		...
		//check basic moves
		if(board[i+direction][j-1]==empty) moves.add(new Move(new Point(i,j),new Point(i+direction,j-1),new ArrayList<Point>()));
		if(board[i+direction][j+1]==empty) moves.add(new Move(new Point(i,j),new Point(i+direction,j+1),new ArrayList<Point>()));
	...
	private ArrayList<Move> checkQueenTake(char[][] board, int i, int j){
		...
		char opponent = currentPlayer == player1 ? player2 : player1;
		int distance =1;
		for(int n=-1; n<=1; n+=2) for(int m=-1; m<=1; m+=2) {
			distance=1;
